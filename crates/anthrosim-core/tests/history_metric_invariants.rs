...
#[test]
fn independent_resource_and_migration_cadences_remain_valid() {
    let mut demography = DemographyConfig::synthetic_validation_v1();
    for band in &mut demography.mortality_bands {
        band.annual_probability_per_million = 0;
    }
    ...
    let migration = MigrationConfig::synthetic_validation_v1().with_decision_periods_per_year(4);

    let checkpoint = Simulation::new(
        ExperimentConfig::new(90, 2)
    ...
    .expect("independent-cadence simulation")
    .checkpoint_at_year(2)
    .expect("two-year checkpoint");

    let terminal_metrics = checkpoint
        .metrics
        .snapshots
        .last()
        .expect("terminal metrics");
    assert_eq!(terminal_metrics.resources.periods_processed, 2);
    assert_eq!(terminal_metrics.migration.decision_boundaries, 8);
    checkpoint
        .validate_invariants()
        .expect("independent M3/M4 clocks must remain valid");
}
