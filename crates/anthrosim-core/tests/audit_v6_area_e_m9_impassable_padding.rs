...
fn unreachable_impassable_padding_cannot_change_local_equal_cost_m9_destination() {
    ...
    for seed in 0..128_u64 {
        ...
        eprintln!(
            "seed={seed} local M9 padding control: baseline={baseline_destination:?} padded={padded_destination:?}"
        );
        assert_eq!(
            padded_destination, baseline_destination,
            "adding one unreachable impassable cell outside the unchanged local M9 problem must not change the selected equal-cost destination"
        );
    }
}
