...
fn restored_aggregate_stock_immediately_below_u64_max_is_representable() {
    let simulation =
        Simulation::new(ExperimentConfig::new(176, 0).with_world(WorldConfig::new(2, 1)))
            .expect("baseline simulation should construct");
    let mut value =
        serde_json::to_value(simulation.resources()).expect("resource state should serialize");
