...
use anthrosim_core::{
    ...
    HouseholdId, MigrationConfig, PopulationConfig, ResourceConfig, Simulation,
    TemporaryMobilityProgram, TemporaryMobilitySchedule, TemporaryTravelResolution,
    TemporaryTravelTable, TemporaryTriggerTiming, WorldConfig,
};
...
#[test]
fn completed_return_is_visible_to_m4_on_the_same_boundary() {
    let seed = 50_003;
    ...
    if let Some(first_migration_sequence) = active
        .events()
        .events
        .iter()
        .filter(|record| {
