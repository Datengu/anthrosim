...
#[test]
fn derived_analysis_schema_documentation_matches_source_constants() {
    let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let repo_root = manifest_dir
        .parent()
        .and_then(|path| path.parent())
        .expect("anthrosim-cli must live under crates/<name>");
    let script = repo_root.join("scripts/test-derived-analysis-schema-docs.py");
        ...
        match probe {
            Ok(output) if output.status.success() => {
                ...
                assert!(
                    ...
                    "derived-analysis schema documentation check failed with {}\nstdout:\n{}\nstderr:\n{}",
                    python.display(),
