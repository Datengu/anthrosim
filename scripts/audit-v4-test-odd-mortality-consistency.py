#!/usr/bin/env python3
"""Audit-v4 Area M ODD/ODD+D current-state re-verification derivative."""
...
audit_v4 = (ROOT / "docs" / "research" / "audit-v4" / "README.md").read_text(encoding="utf-8")

# Version-drift adaptation only: preserve the frozen Audit-v4/v0.3.4 baseline
# while proving that the living standards-facing surfaces now identify v33.
assert "release tag: `v0.3.4`" in audit_v4
assert "exact tagged commit: `8996e99ffc4c5b91b9e00d1048eedd4227ea1d09`" in audit_v4
assert "executable model semantics: `anthrosim-model-semantics-v25`" in audit_v4
assert "current model semantics v33" in odd
assert "current model semantics v33" in oddd
assert "current model semantics v33" in model

# Original substantive positive controls and scientific oracle from discovery PR #548.
...
print("audit_v4_area_m_frozen_semantics_v25=True")
print(f"audit_v4_area_m_odd_semantics_v33={('current model semantics v33' in odd)}")
print(f"audit_v4_area_m_oddd_semantics_v33={('current model semantics v33' in oddd)}")
...
assert not odd_stale and not oddd_stale, (
    "living ODD/ODD+D still describe pre-v15 annual-boundary background "
