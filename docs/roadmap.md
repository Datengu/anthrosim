...
Scientific audit v2 is complete, and `v0.3.3` preserves that repaired post-v0.3.2 state at `anthrosim-model-semantics-v21`. The audit independently covered Areas A–N, repaired and reverified all 15 AV2 findings, and left no open P0–P3 scientific finding at release preparation. This is stronger convergence evidence, not proof of correctness or empirical validity.

The immediate sequence is therefore **run another fresh independent/adversarial audit against immutable `v0.3.3` -> proceed to question-led model interrogation only if convergence remains strong**. This is still not a predetermined M10 feature package. Focused model interrogation can diagnose existing behaviour without tuning toward a desired outcome; concrete research comparisons should then identify which missing mechanism, uncertainty treatment, observation layer or validation target is actually needed next.
