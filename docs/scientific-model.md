...
**Status:** working specification for living v0.3.6-source development during Scientific Audit-v6 remediation / current model semantics v40 (immutable v0.3.6 release baseline and Audit-v6 discovery target: v35; immutable v0.3.5 release baseline: v33; immutable v0.3.4 release baseline: v25; immutable v0.3.3 release baseline: v21)
...
Under v29, each residence cell's living male parentage candidates are ordered by their persisted person `stochastic_coupling_rank` before the existing age/sex eligibility filter and uniform `demography/parentage` reservoir sample are consumed. Canonical `PersonId` or packed-record order is therefore not a causal parent-selection input; locality, eligibility, uniform selection probability and the independent parentage RNG stream are unchanged.
