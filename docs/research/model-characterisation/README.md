...
The first three records were executed from the CI-built release binary produced by the code-changing `main` run for commit `9f0914e2772cd2f0c6243ab4be8b296f3ce48f30` (`anthrosim-model-semantics-v37`). The fourth, fifth, sixth and eighth through twelfth required configuration dimensions or observability that the ordinary CLI does not expose directly, so they used temporary compact experimental runners that checked out the same exact v37 core source commit and called the unchanged core simulation path while retaining only the measurements needed for each sensitivity question.
