...
- **Model analysis:** unchanged except that future relocation-risk/terrain sensitivity can now operate on the intended causal pathway.
- **Output corroboration:** unchanged / not established.

Closing #186 therefore removes one blocking scientific-behaviour defect; it does not upgrade AnthroSim to empirically research-ready status.
