...
| Coverage state | **Areas A–F complete; Area G in progress by overlapping agent/PR #408; Area H complete; Area I in progress with AV3-007/008; Area J in progress with AV3-009; K–N pending** |
...
|---|---|---|---|
| A | Authoritative semantics and scheduler behaviour | **complete — P1 finding open** | Fresh M3/M9 half-open boundary-collision adversary passed. Cross-host parity adversary demonstrated AV3-001/#387: identical declared founder reproductive history yields 0 births in `Simulation` vs 1 birth in `SpatialLandscapeSimulation`. Supplementary 10-seed × 4-year neutral spatial-host parity PR #391 passed before closure. |
| B | Demography, fertility, mortality, ageing, population structure | **complete — P2 findings open** | Fresh certain-fertility M9 locality case confirmed persistent-residence parentage and exposed AV3-002/#392 in M9 replay. Independent founder-history adversary demonstrated AV3-003/#396: `lastBirthDay=-2000` plus an explicit child at day -100 creates 1 artificial first-boundary birth where coherent spacing requires 0. |
| C | Households, kinship, social links, lifecycle structure | **complete — P2 finding open** | PR #397 demonstrated AV3-004/#399: two isomorphic founder kin graphs differing only by canonical-ID relabelling leave **2 vs 1** co-resident living parents after fission. Existing core tests passed before the intended adversarial assertion failed. |
...
| E | Spatial landscape, movement, migration, temporary mobility, and boundaries | **complete — no new finding** | Closed test-only PR #404 head `dc0e5ded1923ab4ef67e88607b4a2112a08d316b`, CI `33475847586`: fresh 90° metamorphic rotation of an asymmetric non-flat 5×5 M9 routing case preserved minimum cost **8000** and **3 outbound + 3 return days**. |
| F | Aggregation and interaction mechanisms | **complete — no new finding** | Closed test-only PR #406 head `5befc83204ae3d534001f9d5e2396fe8e61eec9c`, CI `33480471002`: one seven-day one-person visit produced **7 visitor-person-days / peak 1 / visitor need 7**; two simultaneous visits produced exactly **14 / 2 / 14**. |
| G | Initialization, burn-in, path dependence, continuation state | **in progress by overlapping agent / PR #408; AV3-001/002/003 cross-cutting** | PR #408 independently attacks initialization persistence and checkpoint continuation. This ledger owner must not duplicate that work while the PR remains active. |
...
| I | Sensitivity, uncertainty, convergence, robustness | **in progress — P1/P2 findings open; AV3-005/007/008 cross-cutting** | AV3-007/#413: support-sensitivity reporting accepts fabricated analysis identities and can certify `materialScaleDependence=false` without proving alternatives ran. Concurrent fresh source/2×2 design analysis demonstrated AV3-008/#415 (**P1**): ancestor/descendant research-dimension paths can overwrite one another in declaration order, leaving **4 recorded coordinate combinations but only 2 distinct executable treatments**, so a declared sensitivity factor can be erased while metadata says it varied. Area I remains incomplete pending remaining horizon/initialization/replicate/numerical-convergence robustness coverage as applicable. |
| J | Identifiability, equifinality, calibration, discrimination | **in progress — P1 finding open; AV3-006/007/008 cross-cutting** | Fresh first-principles inspection of frozen `docs/research/identifiability-equifinality-v1.md` and exact analyzer blob `ef90fc82c28ab07b2995512197680f095cc883a7` demonstrated AV3-009/#416 (**P1**). `point.structure` has no enforced canonical type and is reduced using `str(...)`; two compatible structures JSON `1` and `"1"` both become `"1"`, so `compatibleStructures=["1"]`, `identified=true`, `equifinal=false` and a structural-hypothesis claim can pass although two distinct accepted identifiers remain compatible. The same coercion also merges held-out discrimination groups. Area J remains incomplete pending further fresh parameter-compensation, profile/conditional, calibration/held-out-leakage and discrimination attacks. |
| K | Experiment orchestration, configuration, provenance, reproducibility | **pending; AV3-002/007/008 cross-cutting** | AV3-002 blocks declared-founder + M9 replay validation. AV3-007 leaves support-analysis identities unbound. AV3-008 can preserve a declared coordinate in experiment metadata after a later dimension has erased its executable treatment. |
| L | Observability, analysis outputs, statistical summaries | **pending; AV3-002/006/007/009 cross-cutting** | AV3-006 can emit spuriously narrow comparative Monte Carlo precision; AV3-007 can identity-seal unsupported scale robustness; AV3-009 can collapse distinct structural identifiers in equifinality and held-out summaries. |
| M | Documentation, TRACE/ODD/ODD+D, and claim consistency | **pending** | — |
| N | Cross-system integration | **pending; AV3-001 through AV3-009 cross-cutting** | Existing findings span initialization→demography, replay, household topology→resources/movement, latent resource condition, stochastic precision, sensitivity execution/provenance, dimension interaction, and identifiability/structural interpretation. Explicit coupled attacks remain required. |
...
| AV3-003 — declared `lastBirthDay` can predate a later explicitly declared child | **P2** | B/G primary; N cross-cutting | **demonstrated; open; deliberately unrepaired** | #396 | Red PR #395 head `662bf03c1b6b3908adea02e1f8f118d833404c7b`: contradictory declared chronology produces **1 vs expected 0** first-boundary births. |
...
| AV3-007 — support-sensitivity report can claim executed alternatives with fabricated analysis identities | **P2** | I primary; J/K/L/N cross-cutting | **demonstrated; open; deliberately unrepaired** | #413 | Closed red PR #412 head `6c145969f8e11a042f623c1582e59141b77b42c9`, CI `33488012229`: nonexistent primary/alternative analysis identities accepted and `materialScaleDependence=false` certified. |
| AV3-008 — overlapping research dimensions can erase declared sensitivity treatments | **P1** | I primary; J/K/N cross-cutting | **demonstrated; open; deliberately unrepaired** | #415 | Frozen `ResearchExperimentDefinition` rejects exact duplicate paths only. Child `/experiment/resources/annualNeedUnitsPerPerson` values 100/200 followed by parent `/experiment/resources` alternatives containing annual need 300/400 yields **4 recorded coordinates but only 2 executable treatments**; declaration order changes realized design. |
| AV3-009 — structural-identifiability gate can collapse distinct typed structure identifiers | **P1** | J primary; L/N cross-cutting | **demonstrated; open; deliberately unrepaired** | #416 | Exact frozen/current analyzer blob `ef90fc82c28ab07b2995512197680f095cc883a7`. Two compatible points with structure JSON `1` and `"1"` are both stringified to `"1"`; structural diagnostic therefore reports one identified structure and can pass the structural claim. |
...
- Concurrent work then demonstrated AV3-008/#415 (P1): overlapping ancestor/descendant dimension paths can erase a declared treatment while preserving its recorded coordinate and nominal factorial identity.
- Area I remains **in progress**; do not treat AV3-007/008 alone as complete coverage.

### 2026-09-01 — Area J progress / AV3-009 P1

- Live `main` reconstructed at `7ca6c243d4cbad2355c0a78a579500f4ac4e71d4` before this documentation update. Open overlapping audit PRs were #408 (Area G) and stale #394 (Area C test evidence); Area I had just received #415 with no ledger update yet, so this session avoided duplicating that attack and moved to Area J.
- Re-read the Area-J protocol requirements and inspected frozen `docs/research/identifiability-equifinality-v1.md` plus `scripts/research-identifiability.py` from first principles. The analyzer blob is exactly `ef90fc82c28ab07b2995512197680f095cc883a7` on both frozen v0.3.3 and current main, establishing causal equivalence for source evidence.
- Fresh falsification question: can two structurally distinct accepted design-point identifiers collapse during the very gate intended to prevent structural equifinality from being mistaken for identification?
- The point validator imposes no canonical type on `structure`; `_structural_diagnostic` and held-out grouping both use `str(point.get("structure", "default"))`.
- Deterministic two-point limiting case: both points exactly match calibration (`score=0 ± 0`), one declares structure JSON number `1`, the other string `"1"`; both remain compatible, but Python maps both to string `"1"`. The diagnostic therefore has one compatible structure, `identified=true`, `equifinal=false`, and the structural gate can pass.
- This demonstrates AV3-009/#416 (**P1**) because a research-facing mechanism-selection gate can certify unique structural identification while two distinct accepted structure identifiers remain compatible. The same coercion also merges held-out discrimination groups.
- No repair was made. Area J remains **in progress** pending additional fresh attacks on parameter compensation, profile/conditional summaries, calibration/held-out leakage, and discriminating-prediction semantics.
...
Then verify live `main`, immutable `v0.3.3`, open issues/PRs and overlapping work. Do not duplicate Area G while PR #408 remains active. Reconcile any newer Area-I work before touching I. **Continue Area J from first principles if no other agent has taken it; AV3-009 alone does not complete Area J.** Treat AV3-001 through AV3-009 as known cross-cutting limitations, but repair none of them. Create issues for fresh demonstrated defects and update this ledger before handoff. Do not begin site empirical work, release work, or unrelated development.