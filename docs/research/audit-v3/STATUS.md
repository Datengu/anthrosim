...
| Coverage state | **Areas A–F complete; Area G in progress by overlapping agent/PR #408; Area H complete; Area I in progress with AV3-007/008; Area J complete with AV3-009/010/011/012/013; K–N pending** |
...
| J | Identifiability, equifinality, calibration, discrimination | **complete — P1/P2 findings open** | Frozen identifiability analyzer produced AV3-009/#416 typed-structure collapse, AV3-010/#418 non-conservative held-out averaging, AV3-011/#419 false identification of an unvaried parameter, and AV3-012/#421 hidden nuisance equifinality. Fresh calibration/held-out firewall inspection then demonstrated AV3-013/#423 (**P1**): `evidenceId` is only a free non-empty protocol string and is not bound to an authoritative EvidenceCatalog/source identity, so one underlying source can use separate calibration and held-out aliases for the same observable and evade exact-pair circularity detection. The normative contract explicitly calls independent corroboration a stronger claim while acknowledging source reuse; therefore identity aliasing can falsely elevate an in-sample target to independent corroboration. Conditional/nuisance compensation is covered by AV3-012 and the calibration/held-out leakage surface by AV3-013; Area J is complete with limitations recorded. |
...
- Re-read the scientific audit protocol, audit-v3 charter and authoritative ledger. Open PR #408 still owns Area G; stale Area-C evidence PR #394 also remains open, so neither surface was duplicated.
- Re-read frozen `docs/research/evidence-role-firewall-v1.md` and `scripts/research-evidence-role-audit.py` at exact v0.3.3.
- Fresh calibration/held-out leakage attack found that `evidenceId` is accepted as a free non-empty string; confirmatory circularity is checked only on exact `(evidenceId, observableId)` equality. There is no authoritative EvidenceCatalog/source-identity binding in this validation path.
- Therefore one underlying source can be represented by alias A for calibration of observable Y and alias B for `independent_corroboration` of Y, with a matching held-out entry for alias B, and the machine firewall has no identity basis on which to detect the reuse. Demonstrates AV3-013/#423 (**P1**).
- This is distinct from the contract's permitted, disclosed reuse across genuinely different targets: the adversary is the same underlying source and same observable hidden behind protocol-local aliases.
- No repair was made. Together with AV3-009/010/011/012, this supplies fresh structural, parameter, nuisance-compensation, held-out discrimination and calibration/held-out leakage coverage. **Area J is complete with findings open.**

## Handoff instruction

Read the protocol, charter and this ledger; verify live `main`, immutable `v0.3.3`, open issues/PRs and overlapping work. Do not duplicate Area G while PR #408 remains active. Reconcile newer Area-I work before touching I. With Area J complete, the next non-overlapping pending surface is **Area K — experiment orchestration, configuration, provenance and reproducibility**, unless another agent has taken it. Treat AV3-001 through AV3-013 as known cross-cutting limitations and repair none of them. Do not begin Bulstrode empirical work, release work, or unrelated development.