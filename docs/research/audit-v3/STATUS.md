...
| Coverage state | **Areas A–F complete; Area G next / in progress; H–N pending** |
...
Then verify live `main`, immutable `v0.3.3`, open issues/PRs and overlapping work. **Continue Area G from first principles against frozen v0.3.3.** Do not repeat Areas A–F unless new evidence requires it. Treat AV3-001/#387, AV3-002/#392 and AV3-003/#396 as known cross-cutting limitations while testing initialization, burn-in, path dependence and continuation state, but do not repair them. Create issues for any new demonstrated findings and update this ledger. Do **not** repair any audit-v3 finding until the complete A–N discovery pass is finished.
