...
| Current P1 findings | **5 open — AV3-001 / #387; AV3-006 / #410; AV3-008 / #415; AV3-009 / #416; AV3-011 / #419** |
...
| Coverage state | **Areas A–F complete; Area G in progress by overlapping agent/PR #408; Area H complete; Area I in progress with AV3-007/008; Area J in progress with AV3-009/010/011/012; K–N pending** |
...
| I | Sensitivity, uncertainty, convergence, robustness | **in progress — P1/P2 findings open; AV3-005/007/008/011 cross-cutting** | AV3-007/#413: support-sensitivity reporting accepts fabricated analysis identities and can certify `materialScaleDependence=false` without proving alternatives ran. Concurrent Area-I work demonstrated AV3-008/#415 (**P1**): ancestor/descendant research-dimension paths can overwrite one another in declaration order, leaving **4 recorded coordinate combinations but only 2 distinct executable treatments**. AV3-011/#419 further shows the identifiability gate can call a parameter identified when the sensitivity design never varied it. Area I remains incomplete pending additional horizon/initialization/replicate/numerical-convergence robustness coverage as applicable. |
| J | Identifiability, equifinality, calibration, discrimination | **in progress — P1/P2 findings open; AV3-006/007/008 cross-cutting** | Fresh inspection of frozen `docs/research/identifiability-equifinality-v1.md` and exact analyzer blob `ef90fc82c28ab07b2995512197680f095cc883a7` has now demonstrated four independent failures. AV3-009/#416 (**P1**): `point.structure` lacks canonical type validation and `str(...)` collapses JSON `1` and `"1"`, allowing `identified=true` with two distinct accepted structure values. AV3-010/#418 (**P2**): documented conservative held-out structural envelopes are implemented as averages; A=`{0,10}`, B=`{9,11}`, tolerance 1 has true envelope gap 0 but implementation gap 5 and `discriminating=true`. AV3-011/#419 (**P1**): a claimed numeric parameter fixed at `theta=7` at every evaluated point has explored range `[7,7]`; the analyzer special-cases the zero denominator to normalized width `0.0`, so the parameter is certified `identified=true` despite zero treatment variation. AV3-012/#421 (**P2**): two distinct compatible parameter combinations can remain, with nuisance normalized compatible width **1.0**, while a narrower claimed `theta` passes at width **0.1** and the top-level summary still reports `equifinality.present=false`. Area J remains incomplete pending additional calibration/held-out leakage and any remaining conditional-identifiability attacks not already covered by AV3-012. |
...
| L | Observability, analysis outputs, statistical summaries | **pending; AV3-002/006/007/009/010/012 cross-cutting** | AV3-006 can emit spuriously narrow comparative precision; AV3-007 can identity-seal unsupported scale robustness; AV3-009 can collapse structural identities; AV3-010 can turn overlapping compatible held-out ranges into a false discriminating prediction; AV3-012 can suppress explicit equifinality despite visible nuisance-parameter compensation. |
| M | Documentation, TRACE/ODD/ODD+D, and claim consistency | **pending** | — |
| N | Cross-system integration | **pending; AV3-001 through AV3-012 cross-cutting** | Existing findings span initialization→demography, replay, household topology→resources/movement, latent condition, stochastic precision, sensitivity execution/provenance, dimension interactions, identifiability design adequacy, structural interpretation, and nuisance-parameter equifinality reporting. Explicit coupled attacks remain required. |
...
| AV3-011 — constant unvaried numeric parameter can be certified as identified | **P1** | J primary; I/N cross-cutting | **demonstrated; open; deliberately unrepaired** | #419 | Same frozen analyzer blob. With claimed `theta=7` at all evaluated compatible points, explored and compatible ranges are both `[7,7]`; zero denominator is forced to normalized width `0.0`, yielding `identified=true` despite no experimental variation in theta. |
| AV3-012 — equifinality summary can hide nuisance-parameter compensation | **P2** | J primary; L/N cross-cutting | **demonstrated; open; deliberately unrepaired** | #421 | Closed red PR #420 head `603d189e829a3751777d1f04102e005b0881ec12`, CI `33495291689`, job `99815943229`: research gate passes with two compatible points, claimed `theta` normalized width **0.1**, nuisance width **1.0**, yet `equifinality.present=false`. |

## Convergence accounting

The project objective is to achieve a fresh full-scale audit with **no newly discovered P0/P1 finding**.

- AV3-001, AV3-006, AV3-008, AV3-009 and AV3-011 mean v3 is necessarily a **non-clean convergence pass**.
...
### 2026-09-01 — Area J progress / AV3-009 through AV3-011

- Live `main` was `7ca6c243d4cbad2355c0a78a579500f4ac4e71d4` when this Area-J pass began. Open audit PR #408 owns Area G; Area I had just received #415, so this session avoided duplicating either surface and moved to J.
...
- Fresh falsification 1: two calibration-compatible points declare distinct structure JSON values `1` and `"1"`. String coercion collapses both to `"1"`; structural diagnostic reports one compatible structure, `identified=true`, `equifinal=false`. Demonstrates AV3-009/#416 (**P1**).
- Fresh falsification 2: contract says held-out predictions use conservative envelopes, but implementation averages pointwise interval bounds. Deterministic A=`{0,10}`, B=`{9,11}`, tolerance 1 has true envelope gap **0** but implementation gap **5** and `discriminating=true`. Demonstrates AV3-010/#418 (**P2**).
- Fresh falsification 3: claimed numeric `theta=7` is held constant at every evaluated compatible point. The analyzer converts the zero explored-range denominator to normalized width `0.0`, so `identified=true` and the parameter gate can pass despite zero treatment contrast. Demonstrates AV3-011/#419 (**P1**).
- No repair was made. Area J remains **in progress** pending additional fresh parameter-compensation/profile/conditional and calibration/held-out leakage attacks.

### 2026-09-01 — Area J progress / AV3-012

- Live `main` was `8ad1889fea84e26fa2eeee1f6e4c810f6ee6807f`. PR #408 still owned Area G, so this run continued non-overlapping Area J work.
- Re-read the frozen identifiability contract and analyzer blob `ef90fc82c28ab07b2995512197680f095cc883a7` from v0.3.3; the current `main` analyzer blob remained identical.
- Fresh nuisance-compensation adversary preserved in closed red PR #420 head `603d189e829a3751777d1f04102e005b0881ec12`, workflow `33495291689`, job `99815943229`.
- Existing analyzer self-test passed. The adversary then produced: research gate **pass**; compatible points `compatible-a` and `compatible-b`; claimed `theta` normalized compatible width **0.1** and `identified=true`; nuisance normalized compatible width **1.0** and `identified=false`; top-level `equifinality.present=false`.
- This contradicts the frozen documented reporting rule that multiple compatible parameter combinations remain explicit equifinality. Demonstrates AV3-012/#421 (**P2**).
- PR #420 was closed unmerged after preserving the intentionally failing evidence. No production repair was made.
- Area J remains **in progress**; next fresh work should focus on calibration/held-out leakage or any remaining conditional-identifiability failure mode not already covered by AV3-012.
...
Then verify live `main`, immutable `v0.3.3`, open issues/PRs and overlapping work. Do not duplicate Area G while PR #408 remains active. Reconcile newer Area-I work before touching I. **Continue Area J from first principles if no other agent has taken it; AV3-009/010/011/012 do not by themselves complete Area J.** Treat AV3-001 through AV3-012 as known cross-cutting limitations, but repair none of them. Create issues for fresh demonstrated defects and update this ledger before handoff. Do not begin site empirical work, release work, or unrelated development.
