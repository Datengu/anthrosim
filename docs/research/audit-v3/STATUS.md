...
| AV3-016 — TRACE-linked demographic baseline narrative reports superseded v1/64-seed evidence as current | **P2** | M primary; L/N cross-cutting | **demonstrated; open; deliberately unrepaired** | #431 | Living baseline narrative/TRACE uses historical v1, 64 seeds and positive-fission N240 **28.9** / mate limitation **38.7%** / effect **-79.4**; frozen current v2/130-seed result gives N240 **20.923** / mate limitation **43.289%** / simple arm-mean effect **-86.385**. |
| AV3-017 — current scientific-model specification names superseded household-fission v1 semantics | **P2** | M primary; C/N cross-cutting | **demonstrated; open; deliberately unrepaired** | #432 | `docs/scientific-model.md` labels itself current v21 but names `deterministic_size_fission_v1` as the explicit lifecycle alternative; frozen v21 executable/config and current lifecycle contract define active `deterministic_dependency_fission_v2`, which superseded v1's record-order-sensitive partition. |
