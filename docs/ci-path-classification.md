...
AnthroSim is introducing conservative path-aware CI under issue #317. The classifier is now enforced by the cross-platform determinism, spatial-mechanism, landscape-loading, landscape-preprocessing and run-bundle-pack workflows while the remaining required workflows continue to run their existing full behavior.
...
`Cross-platform determinism`, `Spatial mechanism determinism`, `Landscape loading determinism`, `Landscape preprocessing` and `Run bundle pack` are the required workflows currently wired to the reviewed risk classes. On pull requests classified `audit_status_only` or `scientific_documentation_only`, their protected execution contexts still run and resolve successfully, but report an explicit documentation-only N/A disposition instead of running executable simulation/preprocessing/bundle fixtures, uploading golden artifacts or comparing those artifacts.
