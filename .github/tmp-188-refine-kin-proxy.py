...
old = """    /// Every unique cell containing a living direct parent of a living household member.
    ///
...
"""
new = """    /// Every unique persistent-residence cell connected to a household by a living
...
"""
if old not in text:
    raise SystemExit("kin field comment guard not found")
...
"""
if old not in text:
    raise SystemExit("kin collector guard not found")
text = text.replace(old, new, 1)

start = text.index("    fn declared_parent_role_fixture(")
end = text.index("    #[test]\n    fn same_seed_and_state_produce_identical_moves()", start)
replacement = r'''    fn declared_parent_role_fixture(world: &World, internal_parent_is_female: bool) -> Population {
    ...
    fn kin_weight_alone_rewards_reciprocal_cross_household_first_degree_kin() {
        ...
        assert_eq!(child_to_parent.total_utility - child_to_unrelated.total_utility, 250);
...
Path("docs/research/m4-kin-proxy-v1.md").write_text("""# M4 reciprocal first-degree kin-location proxy v1
...
replacements = {
    ...
    "docs/research/odd-d.md": [
        (
            "Symmetric living-direct-parent-location proxy: co-resident and external parents contribute by location regardless of reproductive-sex role, with no first-record truncation; not a theory of kinship, alliance or social obligation.",
            "Reciprocal cross-household living parent-child location proxy: each first-degree edge links both households symmetrically regardless of reproductive-sex role, same-household ties add no spatial preference, and there is no first-record truncation; not a theory of kinship, alliance or social obligation.",
        ),
        (
...
for filename, edits in replacements.items():
    ...
    for old_s, new_s in edits:
        if old_s not in s:
            raise SystemExit(f"doc guard not found in {filename}: {old_s[:80]!r}")
