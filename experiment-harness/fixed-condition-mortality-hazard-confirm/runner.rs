...
fn founders(condition_permille: u16) -> FounderPopulationDefinition {
    ...
    FounderPopulationDefinition::new(
        format!("fixed-condition-confirm-{condition_permille}-v1"),
        ParameterProvenance::SyntheticValidation,
        FounderGenealogyStatus::Unspecified,
        vec![FounderHousehold { id: household, location: CellId::new(1) }],
        people,
    )
}
...
fn run_cell(condition_permille: u16, resource_periods: u16) -> Result<Vec<Value>, String> {
    let mut rows = Vec::with_capacity(200);
    for seed in 8_201_u64..=8_400_u64 {
        ...
        resources.max_scarcity_mortality_probability_per_million = 200_000;
...
fn main() -> Result<(), Box<dyn Error>> {
    let conditions = [250_u16, 500];
    let periods = [1_u16, 4, 8];
    let mut handles = Vec::new();
    for condition in conditions {
        for period in periods {
            handles.push(thread::spawn(move || run_cell(condition, period)));
        }
    }

    let mut rows = Vec::with_capacity(1_200);
